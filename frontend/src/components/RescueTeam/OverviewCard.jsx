  ...
  return (
    <div className="w-full mt-4">
      <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-wider mb-1.5">
        <span
          className={isDanger ? "text-red-500 animate-pulse" : "text-gray-500"}
        >
          {isDanger ? "Sắp hết hạn phản hồi!" : "Tự động từ chối sau:"}
        </span>
        <span
          className={`text-sm ${isDanger ? "text-red-600" : "text-blue-600"}`}
        >
      ...
      <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden shadow-inner">
        ...
        >
          <div
            className={`absolute inset-0 ${isDanger ? "bg-red-500" : "bg-[#0088FF]"}`}
          />
        </div>
      </div>
    </div>
  ...
  <div className="bg-white rounded-[24px] p-4 shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 mb-2 pointer-events-auto">
    ...
    <div className="flex gap-3 mb-2">
      <div className="flex-1 bg-sky-50/50 rounded-2xl p-3 flex flex-col items-center justify-center border border-sky-100">
        ...
        <p className="text-[9px] text-sky-400 font-bold uppercase mt-1">
          Hoạt động
        </p>
      </div>
      <div className="flex-1 bg-green-50/50 rounded-2xl p-3 flex flex-col items-center justify-center border border-green-100">
        ...
        <p className="text-[9px] text-green-400 font-bold uppercase mt-1">
          Hoàn thành
        </p>
      </div>
    </div>
...
const IncidentCard = ({
        ...
        buttons: (
          <div className="w-full space-y-3">
            <div className="flex justify-between items-center bg-blue-50 px-4 py-2.5 rounded-xl border border-blue-100">
              ...
              <span
                className={`text-sm font-black ${canCheckIn ? "text-green-600" : "text-blue-600"}`}
              >
                {distance !== null
                  ? `${distance.toFixed(0)} mét`
                  : "Đang tính..."}
              </span>
            </div>
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold text-sm transition-all active:scale-95 shadow-lg ${
                canCheckIn
                  ? "bg-green-500 text-white shadow-green-200 animate-pulse" // Hiệu ứng mời gọi khi đủ gần
                  : "bg-gray-200 text-gray-400 cursor-not-allowed"
              }`}
            >
              {canCheckIn
                ? "XÁC NHẬN ĐÃ ĐẾN NƠI"
                : "BẠN CẦN ĐẾN GẦN HƠN (<100M)"}
            </button>
          </div>
        ),
      };
  ...
  return (
    <div
      className={`bg-white rounded-[32px] p-5 shadow-2xl border w-full relative pointer-events-auto overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-500 ${status === "incoming" ? "border-blue-400 ring-4 ring-blue-500/20" : "border-gray-100"}`}
    >
      {cardConfig.headerTitle && (
        <h2
          className={`text-center text-[10px] font-black mb-4 uppercase tracking-[0.2em] flex justify-center items-center gap-1.5 ${cardConfig.headerColor}`}
        >
          {status === "incoming" && <AlertTriangle size={14} />}{" "}
          {cardConfig.headerTitle}
        </h2>
      )}
      ...
      <div className="bg-gray-50/80 rounded-[20px] p-4 mb-4 flex items-center justify-between border border-gray-100">
        <div className="flex gap-3.5 items-center">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner ${status === "incoming" ? "bg-blue-100" : "bg-red-100"}`}
          >
            {status === "new_incident" || status === "incoming" ? (
              <Car
                className={`w-6 h-6 ${status === "incoming" ? "text-[#0088FF]" : "text-red-500"} animate-pulse`}
              />
            ) : (
              <span className="material-icons text-red-500 text-xl font-bold">
                warning
              </span>
            )}
          </div>
      ...
      <div className="flex items-start gap-2.5 mb-5 px-1">
        ...
        <p className="text-[12px] text-gray-600 leading-relaxed font-semibold line-clamp-2">
          {address}
        </p>
      </div>
