...
const IncidentCard = ({
  ...
  const canCheckIn = distance !== null && distance <= 100;
  ...
  switch (status) {
      ...
      cardConfig = {
        ...
        buttons: (
          <div className="flex gap-2 w-full">
            ...
            <button onClick={() => onAction("normal")} className="flex-1 bg-gray-100 text-gray-500 py-3.5 rounded-xl font-bold text-xs">ĐÓNG</button>
          </div>
        ),
      };
      break;
        ...
        buttons: (
          <div className="w-full space-y-3">
            <div className="flex justify-between items-center bg-blue-50 px-4 py-2.5 rounded-xl border border-blue-100">
              <span className="text-[10px] font-bold text-blue-400 uppercase">Khoảng cách:</span>
              <span className={`text-sm font-black ${canCheckIn ? "text-green-600" : "text-blue-600"}`}>
                {distance !== null ? `${distance.toFixed(0)} mét` : "Đang tính..."}
              </span>
            </div>
            {isLeader ? (
              <button disabled={!canCheckIn} onClick={onArrive} className={`w-full py-4 rounded-2xl font-bold text-sm transition-all active:scale-95 shadow-lg ${canCheckIn ? "bg-green-500 text-white shadow-green-200 animate-pulse" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
                {canCheckIn ? "XÁC NHẬN ĐÃ ĐẾN NƠI" : "BẠN CẦN ĐẾN GẦN HƠN (<100M)"}
              </button>
            ) : (
              <div className="py-4 text-center bg-gray-50 rounded-2xl text-[10px] font-bold text-gray-400 uppercase border border-dashed border-gray-200">📢 Đang theo chân Đội trưởng đến hiện trường</div>
            )}
          </div>
        ),
      };
    ...
    case "processing":
      cardConfig = {
        ...
        buttons: isLeader ? (
          <button onClick={() => onAction("done")} className="w-full bg-[#ffb000] text-white py-3.5 rounded-2xl font-bold text-sm active:scale-95 shadow-lg shadow-orange-200">ĐÃ XỬ LÝ XONG</button>
        ) : (
          <div className="w-full py-4 bg-amber-50 text-amber-600 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 border border-amber-100">🛠️ ĐANG TRONG QUÁ TRÌNH XỬ LÝ...</div>
        )
      };
      break;
    ...
    case "done":
      cardConfig = {
        ...
        buttons: isLeader ? (
          <button onClick={onComplete} className="w-full bg-[#34c759] text-white py-3.5 rounded-2xl font-bold text-sm active:scale-95 shadow-lg">VỀ TRẠNG THÁI SẴN SÀNG</button>
        ) : (
          <div className="w-full py-4 bg-green-50 text-green-600 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 border border-green-100"><ShieldCheck size={18} /> ĐANG CHỜ ĐỘI TRƯỞNG CHỐT CA</div>
        )
      };
      break;
...
export const OverviewCard = (props) => {
  return props.appState === "normal" ? <NormalOverviewCard /> : <IncidentCard {...props} status={props.appState} />;
};
