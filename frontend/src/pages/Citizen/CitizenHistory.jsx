...
import { AlertCircle, CheckCircle2, MapPin, Clock, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F2F2F7] font-sans pb-24">
        ...
        <div className="flex bg-[#7676801C] p-0.5 rounded-full">
          ...
          <button 
            ...
            className={`flex-1 py-1.5 rounded-full text-[13px] font-bold transition-all ${
              activeTab === "processing" ? "bg-white shadow-sm text-black" : "text-gray-500"
            }`}
          >
            Đang xử lý
          </button>
          ...
          <button 
            ...
            className={`flex-1 py-1.5 rounded-full text-[13px] font-bold transition-all ${
              activeTab === "completed" ? "bg-white shadow-sm text-black" : "text-gray-500"
            }`}
          >
            Đã hoàn thành
          </button>
      ...
      <div className="px-6 space-y-4">
        ...
        {activeTab === "processing" && processingData.map((item) => (
          <div 
            key={item.id} 
          ...
          >
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="text-black text-[18px] font-bold">{item.type}</span>
              </div>
              <div className={`px-2.5 py-0.5 rounded-lg text-[11px] font-bold ${item.bgColor} ${item.color}`}>
                {item.status}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-gray-400">
                <Clock size={14} />
                <span className="text-[13px]">Mã: #{item.id} • {item.time}</span>
              </div>
        ...
        {activeTab === "completed" && completedData.map((item) => (
          <div 
            key={item.id} 
          ...
          >
            ...
            <div className="flex flex-col flex-1 gap-1">
              <span className="text-black text-[18px] font-bold leading-tight">{item.title}</span>
