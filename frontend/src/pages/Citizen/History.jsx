...
export const CitizenHistory = () => {
        ...
        prev.map((inc) =>
          inc._id === data.id
            ? {
                ...inc,
                status: data.status,
                timeline: data.incident?.timeline || inc.timeline,
              }
            : inc,
        ),
      );
  ...
  useEffect(() => {
    ...
    const handleRescueLoc = (data) => {
      ...
      const assignedTeamId =
        selectedIncident.assignedTeam?._id || selectedIncident.assignedTeam;
  ...
  const getStatusConfig = (s) => {
    ...
    return (
      cfgs[s] || { label: s, color: "text-gray-600", bgColor: "bg-gray-50" }
    );
  };
      ...
      <div className="flex flex-col min-h-screen bg-white animate-in slide-in-from-right duration-300">
        ...
        <div className="mt-[64px] h-72 relative z-0">
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] bg-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2">
            <span className="text-xs font-bold">
              {rescuePos ? "Xe đang tiến đến" : "Chờ xe cứu hộ..."}
            </span>
          </div>
        </div>
        ...
        <div className="p-8">
          <div className="flex justify-between items-center">
            ...
            <span
              className={`text-[10px] px-3 py-1 rounded-full font-bold ${getStatusConfig(selectedIncident.status).bgColor} ${getStatusConfig(selectedIncident.status).color}`}
            >
        ...
        {incidents.length === 0 ? (
          ...
          incidents.map((item) => (
            <div
              ...
              className="bg-white p-5 rounded-3xl shadow-sm border active:scale-95 transition-transform"
            >
              <div className="flex justify-between items-start">
                <h3 className="font-bold text-gray-800">{item.title}</h3>
                <span
                  className={`text-[10px] px-2 py-1 rounded-full font-bold ${getStatusConfig(item.status).bgColor} ${getStatusConfig(item.status).color}`}
                >
