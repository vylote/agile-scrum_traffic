...
// Set true: Chặn GPS thật, để dành bắn API giả lập từ Postman
// Set false: Chạy bằng GPS thật khi demo thực tế
...
export const RescueHome = () => {
  ...
  // State quan trọng: Lưu tọa độ của chính xe này
    ...
    const fetchInitialData = async () => {
      try {
        const [pendingRes, activeRes, teamRes] = await Promise.all([
          ...
          api.get(`/rescue-teams?limit=100`) // Gọi danh sách để fix lỗi 404
        ]);

        setIncidentsQueue(pendingRes.data.result.data || []);
        const activeData = activeRes.data.result.data || [];

        // Lọc tọa độ hiện tại của đội mình
        const allTeams = teamRes.data?.result?.data || [];
        const myTeam = allTeams.find(t => t._id === teamId);

        if (myTeam?.currentLocation?.coordinates) {
          setCurrentPos({
            lat: myTeam.currentLocation.coordinates[1],
            lng: myTeam.currentLocation.coordinates[0]
          });
        }
  ...
  // 2. GPS TRACKING THẬT (Đã tích hợp Cờ Mô phỏng)
  useEffect(() => {
    if (isResting || !teamId || IS_SIMULATION_MODE) return;
    ...
    const watchId = navigator.geolocation.watchPosition(reportLocation, null, {
  ...
  // 3. Lắng nghe tọa độ (của chính mình) từ Postman bắn sang
  useEffect(() => {
          ...
          {isResting ? (
            ...
            <div className="px-4">
              <OverviewCard
                ...
                myRole={myInternalRole}
                currentPos={currentPos} // 🔥 Dữ liệu vị trí để tính 500m
              />
            </div>
          )}
