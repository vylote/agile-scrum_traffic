...
import { 
  Plus, Filter, Edit, Trash2, Search, 
  ShieldCheck, Truck, ChevronLeft, ChevronRight, Loader2 
...
export const Users = () => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => { fetchUsers(1); }, 500);
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-8">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col mt-2 min-h-[600px] overflow-hidden">
            ...
            <div className="overflow-x-auto flex-1">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    <tr><td colSpan="4" className="text-center py-20"><Loader2 className="animate-spin mx-auto text-gray-300" size={32} /></td></tr>
                  ) : (
                    users.map((user) => (
                      <tr key={user._id} className="hover:bg-gray-50/80 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                           <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold w-fit mb-1 ${
                             user.role === USER_ROLES.RESCUE ? 'bg-blue-50 text-blue-600 border border-blue-100' : 'bg-purple-50 text-purple-600 border border-purple-100'
                           }`}>
                              {user.role === USER_ROLES.RESCUE ? <Truck size={12}/> : <ShieldCheck size={12}/>}
                              {user.role}
                           </div>
                           {/* Hiển thị mã đội nếu là Rescue */}
                           {user.rescueTeam && (
                             <span className="text-[10px] text-gray-400 font-mono font-bold ml-1">
                               Team: {user.rescueTeam.code}
                             </span>
                           )}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${user.isActive !== false ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-white">
              <span className="text-sm text-gray-500">Trang {pagination.currentPage} / {pagination.totalPages}</span>
              <div className="flex gap-2">
                <button disabled={pagination.currentPage === 1} onClick={() => fetchUsers(pagination.currentPage - 1)} className="p-2 border rounded-lg hover:bg-gray-50 disabled:opacity-30"><ChevronLeft size={18} /></button>
                <button disabled={pagination.currentPage === pagination.totalPages} onClick={() => fetchUsers(pagination.currentPage + 1)} className="p-2 border rounded-lg hover:bg-gray-50 disabled:opacity-30"><ChevronRight size={18} /></button>
              </div>
            </div>
          </div>
        </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[570px] animate-in zoom-in duration-300">
              <AddAccountForm onClose={() => setIsModalOpen(false)} onSubmit={handleCreateAccount} />
            </div>
          </div>
        )}
      </main>
