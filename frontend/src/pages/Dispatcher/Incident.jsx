...
import toast from "react-hot-toast"; // 🔥 Bổ sung thư viện Toast

import sosAlertSound from "/audio/sos_alert.mp3";

// ─── Hàm Format Giờ Việt Nam (UTC+7) ────────────────────────────────────────
...
// ─── Component Thông báo nổi (SOS Toast) ─────────────────────────────────────
  ...
  <div className="fixed top-10 left-1/2 -translate-x-1/2 z-[200] w-[420px] animate-in fade-in slide-in-from-top-10 duration-500">
    <div className="bg-red-600 text-white p-6 rounded-[32px] shadow-[0_25px_60px_rgba(220,38,38,0.4)] border border-red-400/50 flex items-start gap-4 backdrop-blur-md">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-white/80 font-bold flex items-center gap-1"><Clock size={12} /> Vừa xong • Mã: {incident.code}</p>
      </div>
      <button onClick={onDismiss} className="p-2 hover:bg-white/10 rounded-full transition-all active:scale-90"><X size={20} /></button>
    </div>
  </div>
);
...
const IncidentSidebar = ({ incident, onClose, onCancelled, onAssigned }) => {
  ...
  useEffect(() => { if (tab === "assign") loadAvailableTeams(); }, [tab, loadAvailableTeams]);

  // 🔥 THAY THẾ ALERT BẰNG TOAST KHI ASSIGN
  const handleAssign = async () => {
    ...
    try {
      ...
      toast.success("Đã can thiệp gán đội thành công!");
    } catch (e) {
  ...
  // 🔥 THAY THẾ ALERT BẰNG TOAST KHI HỦY
  const handleCancelAction = async () => {
    if (!cancelReason.trim()) return toast.error("Vui lòng nhập lý do hủy báo cáo rác.");
    setCancelling(true);
    try {
      const res = await api.patch(`/incidents/${incident._id}/status`, {
      ...
      toast.success("Đã đóng và hủy sự cố thành công.");
    } catch (e) {
      toast.error("Lỗi hủy: " + (e.response?.data?.message || e.message));
    } finally {
      setCancelling(false);
        ...
        <div className="flex border-b border-gray-100 px-6 bg-gray-50/50">
          {[
            ...
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`flex items-center gap-2 py-4 px-1 mr-8 text-xs font-black uppercase tracking-wider border-b-2 transition-all ${tab === key ? "border-[#0088FF] text-[#0088FF]" : "border-transparent text-gray-400 hover:text-gray-600"}`}
            >
              <Icon size={14} /> {label}
            </button>
          ))}
                ...
                <div className="mt-10 pt-6 border-t-2 border-dashed border-red-100">
                  ...
                  {!showConfirmCancel ? (
                    <button
                      ...
                      className="w-full py-4 rounded-xl border-2 border-red-100 text-red-500 text-xs font-black uppercase hover:bg-red-50 transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="bg-red-50 p-5 rounded-2xl border border-red-100 animate-in zoom-in-95 duration-200">
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          disabled={cancelling}
                          className="flex-1 py-3 bg-red-600 text-white rounded-xl text-xs font-black uppercase shadow-lg shadow-red-200 disabled:opacity-50"
                        >
                          {cancelling ? "ĐANG XỬ LÝ..." : "XÁC NHẬN HỦY"}
                        </button>
                        <button onClick={() => { setShowConfirmCancel(false); setCancelReason(""); }} className="px-6 py-3 bg-white text-gray-500 border border-gray-200 rounded-xl text-xs font-bold">QUAY LẠI</button>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  <div className="grid gap-3">
                    ...
                    {availableTeams.map((team) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${selectedTeam?._id === team._id ? "border-[#0088FF] bg-blue-50/50 shadow-md" : "border-gray-50 bg-gray-50/50 hover:border-gray-200"}`}
                      >
        ...
        {tab === "assign" && canAssign && (
          <div className="p-6 border-t border-gray-100 bg-white">
            <button
              ...
              className="w-full py-4 rounded-2xl bg-[#0088FF] text-white font-black text-sm disabled:opacity-40 hover:bg-blue-600 transition-all shadow-xl uppercase tracking-widest"
            >
...
export const Incident = () => {
  ...
  // 🔥 Dùng Refs để trị ESLint và tránh Stale Closure
  ...
  const fetchIncidents = useCallback(async (pageNum, currentSearch, currentFilters) => {
    try {
      setLoading(true);
      let queryUrl = `/incidents?page=${pageNum}`;
      if (currentSearch) queryUrl += `&search=${encodeURIComponent(currentSearch)}`;
      if (currentFilters.status) queryUrl += `&status=${currentFilters.status}`;
      if (currentFilters.severity) queryUrl += `&severity=${currentFilters.severity}`;
      if (currentFilters.zone) queryUrl += `&zone=${encodeURIComponent(currentFilters.zone)}`;
  ...
  // Gọi API lần đầu & Debounce khi tìm kiếm / lọc
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      setPage(1); // Reset về trang 1
      fetchIncidents(1, searchTerm, filters);
    }, 500);
    return () => clearTimeout(delayDebounceFn);
  }, [searchTerm, filters, fetchIncidents]);

  // Sang trang
  useEffect(() => {
    if (page > 1) {
       fetchIncidents(page, searchRef.current, filtersRef.current);
    }
  }, [page, fetchIncidents]);
  ...
  useEffect(() => {
    ...
    const handleManualNeeded = (data) => {
      ...
      setIncidents((prev) => {
        ...
        if (exists) {
          return prev.map((inc) => inc._id === data.incident._id ? { ...inc, ...data.incident, needsIntervention: true } : inc);
        }
    ...
    const handleNewIncident = (data) => {
      // 🔥 Socket Handler sẽ đọc Data MỚI NHẤT thông qua Refs
      const isCleanState = 
        page === 1 && 
        !searchRef.current && 
        !filtersRef.current.status && 
        !filtersRef.current.severity && 
        !filtersRef.current.zone;
    ...
    const handleDelete = (data) => {
      ...
      if (selectedIncidentRef.current?._id === deletedId) setSelectedIncident(null);
    };

    const handleUpdated = (data) => {
      setIncidents((prev) =>
        prev.map((inc) => inc._id === data.id ? { ...inc, status: data.status, assignedTeam: data.incident?.assignedTeam, needsIntervention: false } : inc)
      );
      if (selectedIncidentRef.current?._id === data.id) {
        setSelectedIncident((prev) => ({ ...prev, status: data.status, assignedTeam: data.incident?.assignedTeam }));
      }
    };
  ...
  return (
    <div className="flex h-screen w-full bg-[#F5F6FA] font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header className="h-[80px] flex items-center justify-between px-10 bg-white border-b border-gray-100 shrink-0 shadow-sm z-10">
          <div>
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Quản lý sự cố</h2>
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Real-time Monitoring & Intervention Center</p>
          </div>
          <div className="w-[400px]">
            {/* 🔥 THANH TÌM KIẾM ĐÃ ĐƯỢC GẮN STATE */}
            <SearchBar value={searchTerm} onChange={setSearchTerm} />
          </div>
        </header>

        {/* 🔥 THANH CÔNG CỤ LỌC (FILTER TOOLBAR) */}
        <div className="px-10 py-4 bg-white/60 backdrop-blur-md border-b border-gray-100 flex items-center gap-4 z-0">
            ...
            <select 
                ...
                className="px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 outline-none focus:border-blue-500 shadow-sm cursor-pointer"
            >
            ...
            >
                ...
                <option value="CRITICAL">Khẩn cấp SOS (Critical)</option>
            </select>

            {/* Ô NHẬP LỌC KHU VỰC */}
            <input 
                type="text"
                placeholder="Ví dụ: Sóc Sơn, Cầu Giấy..."
                value={filters.zone}
                onChange={(e) => setFilters({...filters, zone: e.target.value})}
                className="px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 outline-none focus:border-blue-500 shadow-sm placeholder-gray-300 w-48 transition-all"
            />

            {(filters.status || filters.severity || filters.zone || searchTerm) && (
               <button 
                  onClick={() => { setFilters({status: "", severity: "", zone: ""}); setSearchTerm(""); }}
                  className="ml-auto px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl text-xs font-black uppercase tracking-wider transition-colors"
               >
                  Xóa bộ lọc
               </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto px-10 py-6 no-scrollbar">
          <div className="max-w-5xl mx-auto space-y-6">
            <div className="flex justify-between items-end mb-2">
              <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em]">Kết quả: {pagination.total} Sự cố</h3>
            </div>
            ...
            {loading ? (
              <div className="flex flex-col items-center py-20"><Loader2 size={40} className="animate-spin text-[#0088FF]" /></div>
               ...
               <div className="flex flex-col items-center py-20 opacity-60">
                 ...
                 <p className="text-sm font-bold text-gray-400">Không tìm thấy sự cố nào phù hợp với bộ lọc.</p>
               </div>
            ) : (
              <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-4">
                          <h4 className="text-lg font-black text-gray-900 uppercase tracking-tight max-w-[400px] truncate">{incident.title}</h4>
                          <StatusBadge status={incident.status} />
                        </div>
                        <div className="flex items-center gap-3 text-[11px] font-bold text-gray-400">
                          <Clock size={14} /> 
                          {/* 🔥 SỬ DỤNG HÀM GIỜ VIỆT NAM Ở ĐÂY */}
                          <span>{formatVNTime(incident.createdAt)}</span>
                          <span>•</span> 
                          <span className="font-mono text-blue-500 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{incident.code}</span>
                          {incident.assignedTeam && (
                            <span className="text-violet-600 uppercase tracking-wider font-black ml-2 px-2 py-0.5 bg-violet-50 rounded-md border border-violet-100">
