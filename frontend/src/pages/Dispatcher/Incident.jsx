  ...
  <div className="fixed top-10 left-1/2 -translate-x-1/2 z-[200] w-[420px] animate-in fade-in slide-in-from-top-10 duration-500">
    <div className="bg-red-600 text-white p-6 rounded-[32px] shadow-[0_25px_60px_rgba(220,38,38,0.4)] border border-red-400/50 flex items-start gap-4 backdrop-blur-md">
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-black uppercase text-[10px] tracking-[0.2em] mb-1 text-red-100">
          Lệnh can thiệp khẩn cấp
        </h4>
        <p className="font-black text-base leading-tight mb-1 truncate">
          {incident.title}
        </p>
...
const IncidentSidebar = ({ incident, onClose, onCancelled, onAssigned }) => {
  ...
  const handleCancelAction = async () => {
    if (!cancelReason.trim())
      return alert("Vui lòng nhập lý do hủy báo cáo rác.");
    setCancelling(true);
    try {
      const res = await api.patch(`/incidents/${incident._id}/status`, {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex shadow-2xl">
      ...
      <div className="w-[480px] h-full bg-white flex flex-col animate-in slide-in-from-right duration-300 shadow-2xl">
        <div
          className={`px-6 pt-8 pb-5 border-b border-gray-100 ${isSOS ? "bg-red-50" : "bg-white"}`}
        >
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] font-black text-gray-400 uppercase tracking-widest">
                {isSOS ? "Khẩn cấp" : TYPE_LABELS[incident.type]}
              </span>
            </div>
            <button
              onClick={onClose}
          ...
          <h2 className="font-black text-gray-900 text-2xl leading-tight uppercase mb-3">
            {incident.title}
          </h2>
                ...
                <div className="mt-10 pt-6 border-t-2 border-dashed border-red-100">
                  ...
                  {!showConfirmCancel ? (
                    ...
                    <div className="bg-red-50 p-5 rounded-2xl border border-red-100 animate-in zoom-in-95 duration-200">
                      <p className="text-xs font-bold text-red-700 mb-3 uppercase">
                        Lý do hủy sự cố:
                      </p>
                      <textarea
                        autoFocus
                      ...
                      <div className="flex gap-3">
                        ...
                        <button
                          ...
                          className="px-6 py-3 bg-white text-gray-500 border border-gray-200 rounded-xl text-xs font-bold"
                        >
                          QUAY LẠI
                        </button>
                      </div>
              ...
              {!canAssign ? (
                <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100 flex items-center gap-4">
                  ...
                  <p className="text-sm text-blue-700 font-bold uppercase">
                    Sự cố đã được gán cho: <br />{" "}
                    <span className="text-lg">{assignedTeamName}</span>
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="grid gap-3">
                    ...
                    {availableTeams.map((team) => (
                      ...
                      >
                        ...
                        <span className="text-[9px] bg-green-100 text-green-700 px-2 py-1 rounded-md font-black uppercase">
                          Available
                        </span>
                      </button>
                    ))}
        ...
        {tab === "assign" && canAssign && (
          <div className="p-6 border-t border-gray-100 bg-white">
            ...
            >
              {assigning
                ? "ĐANG XỬ LÝ..."
                : `Gán đơn cho: ${selectedTeam?.name || "Chọn đội"}`}
            </button>
          </div>
        )}
      </div>
        ...
        if (exists) {
          return prev.map((inc) =>
            inc._id === data.incident._id
              ? { ...inc, ...data.incident, needsIntervention: true }
              : inc,
          );
        }
        ...
        <header className="h-[80px] flex items-center justify-between px-10 bg-white/50 backdrop-blur-md border-b border-gray-100 shrink-0">
          <div>
            ...
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">
              Real-time Monitoring & InterventionCenter
            </p>
          </div>
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            <div className="flex justify-between items-end mb-2">
              <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em]">
                Tất cả báo cáo ({pagination.total})
              </h3>
            </div>
                ...
                {incidents.map((incident) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-6">
                      <div
                        className={`p-4 rounded-2xl ${incident.severity === "CRITICAL" || incident.needsIntervention ? "bg-red-100 text-red-500" : "bg-gray-100 text-gray-400"} group-hover:scale-110 transition-transform`}
                      >
                    ...
                    <ChevronRight
                      className="text-gray-200 group-hover:text-[#0088FF] group-hover:translate-x-1 transition-all"
                      size={24}
                    />
                  </div>
                ))}
            ...
            {pagination.totalPages > 1 && (
              <div className="flex items-center justify-center gap-8 py-10">
                <button
                  ...
                  className="p-3 bg-white rounded-2xl shadow-sm border border-gray-100 disabled:opacity-30 active:scale-90 transition-all"
                >
                  <ChevronLeft />
                </button>
          ...
          <IncidentSidebar
            ...
            onAssigned={(updated) => {
              setIncidents((prev) =>
                prev.map((i) =>
                  i._id === updated._id
                    ? { ...updated, needsIntervention: false }
                    : i,
                ),
              );
              setSelectedIncident(null);
