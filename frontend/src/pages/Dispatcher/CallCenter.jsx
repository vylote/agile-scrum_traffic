...
// Import Icons
import { Phone, Video, Send, Search, MoreVertical, Image as ImageIcon, Paperclip } from "lucide-react";
                ...
                {chatList.map((chat) => (
                  <div 
                    ...
                    className={`flex items-start gap-3 p-4 cursor-pointer transition-colors border-b border-gray-50 ${
                      chat.id === activeChat.id ? "bg-blue-50/50" : "hover:bg-gray-50"
                    }`}
                  >
                    <div className="relative">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        <h4 className="text-sm font-bold text-gray-900 truncate pr-2">{chat.name}</h4>
                        <span className="text-[11px] font-medium text-gray-400 shrink-0">{chat.time}</span>
                      </div>
            ...
            <section className="flex-1 flex flex-col bg-[#F9FAFB]">
              ...
              <div className="h-[72px] px-6 border-b border-gray-200 bg-white flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-gray-900 leading-tight">{activeChat.name}</h3>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                      <span className="text-xs font-medium text-green-600">Trực tuyến</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
                ...
                {mockMessages.map((msg) => (
                  <div key={msg.id} className={`flex max-w-[80%] ${msg.isOutgoing ? "self-end flex-row-reverse" : "self-start"}`}>
                    {!msg.isOutgoing && (
                      <img src={activeChat.avatar} alt="Avatar" className="w-8 h-8 rounded-full object-cover mr-3 shrink-0" />
                    )}
