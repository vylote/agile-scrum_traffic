...
const StaffMember = ({ member }) => (
  <article className="flex gap-3 py-3 px-4 w-full bg-white rounded-2xl border border-gray-200 shadow-sm mb-2">
    ...
    <a
      href={`tel:${member.userId?.phone}`}
      className="self-center p-2 text-green-600 bg-green-50 rounded-full"
    >
      <Phone size={16} />
    </a>
  </article>
...
export const FleetManagement = () => {
  ...
  useEffect(() => {
    fetchRescueTeams(1, activeTab);
  }, [activeTab, fetchRescueTeams]);
  ...
  useEffect(() => {
    ...
    const handleLocationUpdate = (data) => {
      setTeams((prev) =>
        prev.map((t) =>
          t._id === data.teamId ? { ...t, status: data.status } : t,
        ),
      );
    };
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-8">
          ...
          <div className="flex flex-wrap gap-4 mt-2 mb-8">
            {["ALL", "AVAILABLE", "BUSY", "OFFLINE"].map((tabId) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${activeTab === tabId ? "bg-indigo-950 text-white shadow-lg" : "bg-white text-gray-600 border border-gray-100 hover:bg-gray-50"}`}
              >
          ...
          {loading ? (
            <div className="flex justify-center py-20">
              <Loader2 className="animate-spin text-indigo-950" size={40} />
            </div>
          ) : (
              ...
              {teams.map((team) => {
                ...
                return (
                  <div
                    ...
                    className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 hover:shadow-xl transition-all cursor-pointer group"
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div
                        className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase ${ui.color}`}
                      >
                        <span
                    ...
                    <p className="text-xs text-gray-400 font-mono mb-4 uppercase">
                      MÃ: {team.code}
                    </p>
            ...
            <aside className="relative w-full max-w-[420px] bg-[#F2F2F7] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              <div className="p-6 bg-white border-b border-gray-200">
                <div className="flex justify-between items-center mb-4">
                  <div
                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border ${getStatusUI(selectedTeam.status).color}`}
                  >
                ...
                <p className="text-xs text-gray-400 font-mono mt-1 uppercase">
                  Mã: {selectedTeam.code} • Phụ trách: {selectedTeam.zone}
                </p>
              </div>
                ...
                <section>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm text-center">
                    ...
                    <p className="text-sm font-bold text-indigo-600 mt-1">
                      {selectedTeam.currentLocation.coordinates[1]},{" "}
                      {selectedTeam.currentLocation.coordinates[0]}
                    </p>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedTeam.capabilities?.map((cap, i) => (
                      <span
                        key={i}
                        className="px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-xl text-xs font-bold"
                      >
                        {cap}
                      </span>
                    ))}
